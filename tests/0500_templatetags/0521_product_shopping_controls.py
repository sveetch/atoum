...
def test_in_inventory(client, db, initial_catalog, rf, settings):  # noqa: F811
    ...
    assert dom.find("#id_shopping-product-{}_quantity".format(corn.id)).attr("value") == "42"
...
def test_not_in_inventory(client, db, initial_catalog, rf, settings):  # noqa: F811
    ...
    assert dom.find("#id_shopping-product-{}_quantity".format(wing.id)).attr("value") == "1"
...
def test_not_in_shopping_with_different(client, db, initial_catalog, rf, settings):  # noqa: F811
    """
    ...
    context = RequestContext(
        rf,
        {
            ...
            "debug": False,
            "user": user,
            "shopping_inventory": ShoppingListInventory(obj=shopping_inventory),
        }
    )
...
def test_in_shopping_with_different(client, db, initial_catalog, rf, settings):  # noqa: F811
    """
...
def test_not_in_shopping_with_same(client, db, initial_catalog, rf, settings):  # noqa: F811
    """
    ...
    context = RequestContext(
        rf,
        {
            ...
            "debug": False,
            "user": user,
            "shopping_inventory": ShoppingListInventory(obj=shopping_object),
        }
    )
    render = product_shopping_controls(context, wing, shopping=shopping_object)
    dom = html_pyquery(render, rooted=True)
...
def test_in_shopping_with_same(client, db, initial_catalog, rf, settings):  # noqa: F811
    """
...
def test_dev_queryset_count(client, db, initial_catalog, rf, settings, django_assert_num_queries):  # noqa: F811
    """
    TODO: R&D to make efficient way to get shopping items from a shopping object in
